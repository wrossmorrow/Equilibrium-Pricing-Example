...
static double * Z;		// I vector: income values
	...
	for( f = 0 ; f < F ; f++ ) {
		...
		for( j = sJf[f] ; j < sJf[f+1] ; j++ ) { 
			...
			if( p[j] >= maxinc ) { // use extended map instead of what is calculated above
				...
				else { // LAMp[j] should be zero; it should not be positive (except for round-off errors?)
	...
	for( f = 0 ; f < F ; f++ ) {
		...
		for( k = sJf[f] ; k < sJf[f] + Jf[f] ; k++ ) {
			...
			if( LAMp[k] < 0.0 ) { 
				...
				// jac[k,:] <- DpUPL[maxinci,:] * PIL[maxinci,f(k)]
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 * Callback routine for KNITRO execution. This version differs from the examples
 * given by Ziena, in that we use a switch statement to segment the different
 * call types. 
 * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int knitro_callback(const int             evalRequestCode,
					const int             N, // number of variables
					const int             M, // number of "c" constraints
					const int             Jnnz,
					const int             Hnnz,
					...
					void   *        userParams)
{
	...
	switch( evalRequestCode ) {
		...
		default: printf("Unknown KNITRO callback code.\n"); return -1;
			
		case KTR_RC_EVALFC:
			
			obj[0] = 0.0; // feasibility problem
			
			zeta(); // computes phi too
			cblas_dcopy(J, phi, 1, cns, 1);
			
			break;
			
		case KTR_RC_EVALGA:
			
			cblas_dscal(J, 0.0, objGrad, 1); // feasbility problem 
...
void nlstrials(char * rfpn)
{
	int nStatus = 0;
	
	KTR_context_ptr  kc;
	
    int          N, M, Jnnz, objGoal, objType;
    int          *cType;
    double       *Jdata;
	int			 *Jrows, *Jcols;
    double       obj, *x, *lambda;
    double       *xLoBnds, *xUpBnds, *xInitial;
	double		 *cLoBnds, *cUpBnds;
    ...
    cLoBnds      = (double *) calloc (M, sizeof(double)); // initializes to zero
    cUpBnds      = (double *) calloc (M, sizeof(double)); // initializes to zero
    Jdata		 = (double *) calloc (Jnnz, sizeof(double));
    Jrows		 = (int    *) malloc (Jnnz * sizeof(int));
    Jcols		 = (int    *) malloc (Jnnz * sizeof(int));
    x			 = (double *) malloc (N     * sizeof(double));
    lambda		 = (double *) malloc ((N+M) * sizeof(double));
	...
	// initialize KNITRO problem parameters
	objGoal = KTR_OBJGOAL_MINIMIZE;
	objType = KTR_OBJTYPE_LINEAR;
	
	for( j = 0 ; j < J ; j++ ) {
		
		xLoBnds[j]  = - KTR_INFBOUND;
		xUpBnds[j]  =   KTR_INFBOUND;
		xInitial[j] = p0[j];
	...
	kc = KTR_new();
	if( kc == NULL ) { printf ("Failed to find a Ziena license.\n"); return; }
	
	// load options from a file
	if( KTR_load_param_file(kc,"knitro.opt") != 0 ) { 
		printf("KNITRO options file could not be loaded.\n");
	}
	
	// "register" KNITRO problem parameters
	nStatus = KTR_init_problem(kc, 
							   N, 
							   objGoal, 
							   objType,
							   xLoBnds, 
	...
	if( KTR_set_func_callback(kc, &knitro_callback) != 0 ) { exit(-1); }
	if( KTR_set_grad_callback(kc, &knitro_callback) != 0 ) { exit(-1); }
	...
	for( t = 0 ; t < T ; t++ ) {
		...
		if( t > 0 ) {
			...
			for( j = 0 ; j < J ; j++ ) { xInitial[j] = p0[J*t+j]; }
		...
		/*
		// check objective gradient and constraint Jacobian
		ktr_evalga_check(N,
						 M,
						 &knitro_callback,
						 &knitro_callback,
		...
		*/
		...
		printf("Zeta 'NLS' Trial %i -- KNITRO terminated with status %i\n",t+1,nStatus);
		if( soscflag == 1 ) {
			printf("Zeta 'NLS' Trial %i -- KNITRO found a local equilibria\n",t+1);
		} else {
			printf("Zeta 'NLS' Trial %i -- KNITRO did not find a local equilibrium:\n",t+1);
			for( f = 0 ; f < F ; f++ ) {
		...
		if( rfp != NULL ) {
			...
			// then iterations required (only one call in KNITRO 8)
			fprintf(rfp,"%i,",KTR_get_number_iters(kc));
			fprintf(rfp,"%i,",0);

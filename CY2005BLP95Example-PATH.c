...
// mean utility coefficients: log(inc-price) , 10 mi / $ , l*w , hp/wt , og
...
void initprob()
{
	...
	*/
	...
	for( i = 0 ; i < I ; i++ ) { 
		...
		UC[Kp*i+1] = uc_a[1] + uc_c[1] * V[Kp*i+1]; // coefficient on 10 mi / $
		UC[Kp*i+2] = uc_a[2] + uc_c[2] * V[Kp*i+2]; // coefficient on hp / wt
		UC[Kp*i+3] = uc_a[3] + uc_c[3] * V[Kp*i+3]; // coefficient on len * wid
	...
	FILE * finincdata;
	finincdata = fopen( "BLPFIDATA.csv" , "w" );
	
	for( i = 0 ; i < I ; i++ ) {
		fprintf( finincdata , "%0.16f," , Z[i] );
		for( j = 0 ; j < J ; j++ ) {
			fprintf( finincdata , "%0.16f," , W[I*j+i] );
		}
		fprintf( finincdata , "\n" );
	}
...
static CB_FUNC(void) nls_bounds(void*   v, 
								int		N, 
								double* x, 
								double* lb, 
								double* ub)
										...
										double* f)
{
	...
	switch( maptype ) {
		...
		case 'g': // combined gradient (actually negative combined gradient)
		case 'G':
			combgrad();
										...
										double* data)
{
	...
	switch( maptype ) {
		...
		case 'g': // gradient (actually negative gradient)
		case 'G':
		...
		case 'Z':
			...
			zhiphipsi();
			if( wantf ) { cblas_dcopy(J, phi, 1, f, 1); }
			phiders(data);
	...
	/*
	 ...
	 for( j = 0 ; j < J-1 ; j++ ) {
	 for( k = 0 ; k < J-1 ; k++ ) {
	 printf(" %0.4f ,",data[J*k+j]);
	 }
	 printf("%0.4f ;\n",data[J*(J-1)+j]);
	 }
	 for( k = 0 ; k < J-1 ; k++ ) {
	 printf(" %0.4f ,",data[J*k+(J-1)]);
	 }
	 printf("%0.4f ];\n",data[J*J-1]);
	 */
	
	/*
	printf("  Jacobian (CCF) is %i x %i with %i nonzeros:\n",J,J,J*J);
	
	printf("    col lengths: %i",col_len[0]);
	for( j = 1 ; j < J ; j++ ) { printf(" , %i",col_len[j]); }
	printf("\n");
	
	printf("    col starts (FORTRAN-Style): %i",col_start[0]);
	for( j = 1 ; j < J ; j++ ) { printf(" , %i",col_start[j]); }
	printf("\n");
	
	printf("    rows (FORTRAN-Style): %i",row[0]);
	for( j = 1 ; j < J*J ; j++ ) { printf(" , %i",row[j]); }
	printf("\n");
	
	for( j = 0 ; j < J ; j++ ) {
		printf("    Column %i (%i elements): entries %i through %i\n",j+1,col_len[j],col_start[j]-1,col_start[j]-1+col_len[j]-1);
		for( k = col_start[j]-1 ; k < col_start[j]-1 + col_len[j] ; k++ ) {
			printf("  J[%i,%i] = %0.16f\n",row[k],j+1,data[k]);
		}
	}
	 */
...
static MCP_Interface nls_interface =
{
	NULL,
	problem_size, 
	nls_bounds,
...
void mcptrials(char * lfpn, char * sfpn, char * rfpn)
{
	...
	for( t = 0 ; t < T ; t++ ) {
		...
		switch( maptype ) {
			case 'g':
			case 'G':
				printf("CG 'MCP' Trial %i -- Starting PATH... \n",t+1);
				break;
			case 'z':
			case 'Z':
				printf("Zeta 'MCP' Trial %i -- Starting PATH... \n",t+1);
				break;
			default:
				break;
		}
		...
		if( checkders ) {
			...
			path_evalga_check(problem_size, 
							  mcp_bounds, 
							  function_evaluation, 
							  jacobian_evaluation, 
							  NULL, 
							  NULL);
			filled = 0; // now we must reset to make sure PATH gets the right structure
			
		}
		
		// solve problem with PATH
		...
		switch( maptype ) {
			...
			case 'z':
			case 'Z':
				printf("Zeta 'MCP' Trial %i -- PATH terminated with status %i\n",t+1,(int)status);
				if( soscflag == 1 ) {
					printf("Zeta 'MCP' Trial %i -- PATH found a local equilibria\n",t+1);
				} else {
					printf("Zeta 'MCP' Trial %i -- PATH did not find a local equilibrium:\n",t+1);
					for( f = 0 ; f < F ; f++ ) {
						printf("Zeta 'MCP' Trial %i -- firm %i: (%i,%i,%i)\n",t+1,f+1,ppom[f+1],soscinfo[f],soscinfo[F+f]);
						if( soscinfo[2*F+f] != 1 ) { soscflagP = 0; }
					}
				}
				break;
		...
		if( rfp != NULL ) {
			...
			fprintf(rfp,"%i,",info.major_iterations);
			fprintf(rfp,"%i,",info.minor_iterations);
			
			// then time required (total then, % time spent in factorizations)
			fprintf(rfp,"%0.16f,",info.total_time);
			fprintf(rfp,"%0.16f,",100.0*((double)(info.total_time-info.basis_time))/((double)info.total_time));
...
void nlstrials(char * lfpn, char * sfpn, char * rfpn)
{
	...
	for( t = 0 ; t < T ; t++ ) {
		...
		MCP_Jacobian_Structure_Constant(m, 1);
		MCP_SetInterface(m, &nls_interface);
		MCP_SetPresolveInterface(m, &mcp_presolve);
		...
		// define this as a general MCP (even though it is basically a NLS)
		Path_SetProblemClass(m, MCP_MCP);
		
		// enforce zeta maptype
		maptype = 'z';
		
		printf("Zeta 'NLS' Trial %i -- Starting PATH... \n",t+1);
		...
		printf("Zeta 'NLS' Trial %i -- PATH terminated with status %i\n",t+1,(int)status);
		if( soscflag == 1 ) {
			printf("Zeta 'NLS' Trial %i -- PATH found a local equilibria\n",t+1);
		} else {
			printf("Zeta 'NLS' Trial %i -- PATH did not find a local equilibrium:\n",t+1);
			for( f = 0 ; f < F ; f++ ) {
...
int main(int argc, char *argv[])
{
	...
	for( i = 1 ; i < argc ; i++ ) {
		...
		if( (argv[i])[0] == '-' ) {
			...
			switch( (argv[i])[1] ) {
				...
				case 'C': // integral scale
					checkders = 1;
					break;
					
				case 'p': // reading problem type
					probtype = (argv[i])[2];
					switch( probtype ) {
						case 'g':
						case 'G':
							printf("  Solving MCP with Combined Gradient.\n");
							maptype = 'G';
							break;
						case 'n':
						case 'N':
							printf("  Solve nonlinear system with Phi map.\n");
							maptype = 'Z';
							break;
						case 'p':
						case 'P':
	...
	switch( probtype ) {
		...
		case 'G':
		case 'p':
		case 'P': mcptrials(lfpn,sfpn,rfpn); break;
		case 'n':
		case 'N': nlstrials(lfpn,sfpn,rfpn); break;
		default:  break;
	}

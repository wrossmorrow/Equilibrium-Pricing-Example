...
int evalPEQSOSC( int * ppom , int * info , char ** ws )
{
	...
	// also have to compute "phi" here, as that isn't in the zeta function in this code
	//
	//		phi <-  p - c - z 
	// 
...
// use normal map fixed-point iteration
void fpitrials(char * rfpn)
{	
	FILE * rfp = NULL;
	...
	clock_t ticks; // have to do our own timing; is this consistent with PATH's timing?
	
	// "info" vectors for sosc check
	ppom = (int*)calloc( 2*(F+1) , sizeof(int) );
	soscinfo = (int*)calloc( 3*F , sizeof(int) );
	q = (double*)calloc( J , sizeof(double) );
	...
	for( t = 0 ; t < T ; t++ ) {
		...
		// start at p0[:,t] (p <- p0[:,t])
		cblas_dcopy( J , p0 + J*t , 1 , p , 1 );
		
		// start timer
		ticks = clock();
		
		// fixed-point iterations (we allow 500 major iterations in PATH, 
		// so its fair to allocate 500 here even though each fixed-point
		// iteration should be markedly less expensive that each PATH iteration)
		flag = 0;
		for( n = 0 ; n < 500 ; n++ ) {
			
			// store current prices in q and "phi" (see below)
			cblas_dcopy( J , p , 1 , q , 1 );
			cblas_dcopy( J , p , 1 , phi , 1 );
			
			// project prices 
			for( j = 0 ; j < J ; j++ ) {
				if( p[j] < 0.0 ) { p[j] = 0.0; }
				if( p[j] > maxinc ) { p[j] = maxinc; }
			}
			
			// evaluate zeta map at * projected * prices
			// (this obtains the normal map fixed-point iteration)
			markups();
			utilities();
			probabilities();
			lamgam();
			zeta(); // (does * not * include phi(p) = p - c - z(p) calculation)
			
			// form "phi" <- p - c - zeta(proj(p))
			cblas_daxpy( J , -1.0 , c , 1 , phi , 1 );
			cblas_daxpy( J , -1.0 , z , 1 , phi , 1 );
			
			// test convergence (using step, not combined gradient)
			nrm = fabs( phi[ cblas_idamax( J , phi , 1 ) ] );
			
			// if we have converged, stop. 
			// if we haven't converged, replace prices
			if( nrm > 1.0e-6 ) { 
				cblas_dcopy(J, c, 1, p, 1);
				cblas_daxpy(J, 1.0, z, 1, p, 1);
			} else { 
				flag = 1;
				break;
			}
		...
		if( flag == 1 ) { 
			...
			// now check sosc
			soscflag = evalPEQSOSC( ppom , soscinfo , &ws );
		} else { soscflag = 0; }
		soscflagP = 1;
		
		printf("Zeta 'FPI' Trial %i -- FPI terminated (in %i iterations) with status %i\n",t+1,n,flag);
		if( soscflag == 1 ) {
			printf("Zeta 'FPI' Trial %i -- FPI found a local equilibria\n",t+1);
		} else {
			printf("Zeta 'FPI' Trial %i -- FPI did not find a local equilibrium:\n",t+1);
			for( f = 0 ; f < F ; f++ ) {
				printf("Zeta 'FPI' Trial %i --   firm %i: (%i,%i,%i),(%i,%i)\n",t+1,f+1,ppom[1+f],soscinfo[f],soscinfo[F+f],ppom[F+2+f],soscinfo[2*F+f]);
		...
		// if there are warnings, print them out and reset ws for next trial
		if( ws != NULL ) { printf("%s",ws); free( ws ); ws = NULL; }
		...
		if( rfp != NULL ) {
			...
			// then iterations required (major only, no minor)
			fprintf(rfp,"%i,",n);
			fprintf(rfp,"%i,",0);
			
			// then time required (total only, 0% time spent in factorizations)
			...
			// first is w.r.t. prices, second is w.r.t. probabilities

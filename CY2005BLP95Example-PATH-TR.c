...
static double e0; // initial (can be set as a command-line argument)
static double e; // "actual"
...
static CB_FUNC(void) start(void *v) { filled = 0; }

static CB_FUNC(void) problem_size(void *v, int *n, int *nnz)
{
	n[0]   = J;
	nnz[0] = J*J; // Jacobians are (assumed to be) dense
}

static CB_FUNC(void) mcp_bounds(void*   v, 
							int		N, 
							double* x, 
							double* lb, 
							double* ub)
...
static CB_FUNC(int) function_evaluation(void*   v, 
										int		N, 
										double* x, 
										double* f)
{
	...
	// combined gradient (actually negative combined gradient)
	combgrad();
	cblas_dcopy(J, cg, 1, f, 1);
	cblas_dscal(J, -1.0, f, 1);
	
	// Tikhonov regularization
	for( j = 0 ; j < J ; j++ ) { f[j] += e * p[j]; }
										...
										double* data)
{
	...
	// if( wantf ) { printf("Evaluating Jacobian (with f)...\n"); }
	// else { printf("Evaluating Jacobian...\n"); }
	...
	cblas_dscal(J*J, -1.0, data, 1); // using NEGATIVE combined gradient in the MCP
	
	// Tikhonov regularization (add scaled identity matrix)
	for( j = 0 ; j < J ; j++ ) { data[J*j+j] += e; }
	...
	if ( filled == 0 ) {
		...
		// printf("  Filling in Jacobian Structure...\n");
		
		// J columns, each of length J. 
		// -- STRUCTURE MUST BE IN FORTRAN-STYLE INDEXING -- 
		for( j = 0 ; j < J ; j++ ) { 
			col_len[j]   = J;
			col_start[j] = J*j+1;
			for( k = 0 ; k < J ; k++ ) { row[J*j+k] = k+1; }
		}
...
void mcptrials(char * lfpn, char * sfpn, char * rfpn)
{
	Options_Interface *o;
	MCP *m;
	...
	int triters, totmajiters, totminiters;
	double tottime, totbasistime;
	...
	for( t = 0 ; t < T ; t++ ) {
		...
		// define PATH license (as provided by Todd Munson, ANL)
		License_SetString("2794369445&William_Morrow&Department_of_Economics&Iowa_State_University&USR&15674&3_3_2010&1000&PATH&GEN&0_0_0&0_0_0&5000&0_0");
		...
		Output_Printf(Output_Log | Output_Status | Output_Listing,
					  ...
					  J, J*J, 100.0*J*J/(1.0*J*J));
		
		m = MCP_Create( J , J*J+1 );
		...
		// define this as a general MCP
		Path_SetProblemClass(m, MCP_MCP);
		
		printf("CG 'MCP' Trial %i -- Starting TR-PATH... \n",t+1);
		
		// initial TR parameter
		e = e0;
		...
		// path_evalga_check(problem_size, mcp_bounds, function_evaluation, jacobian_evaluation, NULL, NULL);
		
		while( e >= 0.0 ) {
			
			printf("  Starting PATH(%0.4f)...  ",e);
			
			// solve regularized problem with PATH
			// this call will start at p0 as defined in bounds(...)
			status = Path_Solve(m, &info);
			
			// increment regularization iteration counter
			triters++;
			
			// accumulate iterations
			totmajiters += info.major_iterations;
			totminiters += info.minor_iterations;
			
			// accumulate time
			tottime += info.total_time;
			tottime += info.basis_time;
			
			// get access to solution and copy into p vector
			x = MCP_GetX(m);
			cblas_dcopy( J , x , 1 , p , 1 );
			
			// act based on solve success/failure
			switch( status ) {
					
				case MCP_Solved: 
					
					printf("PATH(%0.4f) succeeded",e);
					
					// if we solved unregularized problem, terminate
					...
					// otherwise... 
						...
						// compute norm of step between this and last solve
						// we use p0 for this trial as storage
						cblas_daxpy( J , -1.0 , p , 1 , p0 + J*t , 1 );
						n = ABS( p0[ J*t + cblas_idamax(J,p0+J*t,1) ] );
						printf("... step norm %0.4f\n",n); 
						
						if( n <= 1.0e-4 ) {
							
							// if step between last two solutions was "small", 
							// try solving unregularized problem starting at 
							// the current solution
							e = 0.0;
							cblas_dcopy( J , p , 1 , p0 + J*t , 1 );
							
						} else {
							
							// if step was "large", reduce regularization
							// parameter and solve that problem
							e = e / 10.0;
							cblas_dcopy( J , p , 1 , p0 + J*t , 1 );
				...
				case MCP_NoProgress: // A stationary point was found
					
					printf("  PATH(%0.4f) failed: found a stationary point.\n",e);
					if( e > 0.0 ) { 
						e = e / 10.0;
						cblas_dcopy( J , p , 1 , p0 + J*t , 1 );
					} else {
				...
				case MCP_MajorIterationLimit: // Major iteration limit met
					printf("  PATH(%0.4f) failed: major iteration limit met.\n",e);
					e = -1.0;
					break;
				case MCP_MinorIterationLimit: //Cumulative minor iterlim met
					printf("  PATH(%0.4f) failed: cumulative minor iteration limit reached.\n",e);
					e = -1.0;
					break;
				case MCP_TimeLimit: // Ran out of time
					printf("  PATH(%0.4f) failed: ran out of time.\n",e);
					e = -1.0;
					break;
				case MCP_UserInterrupt: // Control-C, typically
					printf("  PATH(%0.4f) failed: solve killed.\n",e);
					e = -1.0;
					break;
				case MCP_BoundError: // Problem has a bound error 
					printf("  PATH(%0.4f) failed: bound error.\n",e);
					e = -1.0;
					break;
				case MCP_DomainError:           /* Could not find starting point                */
					printf("  PATH(%0.4f) failed: could not find starting point.\n",e);
					e = -1.0;
					break;
				case MCP_Infeasible:	     /* Problem has no solution                      */
					printf("  PATH(%0.4f) failed: problem infeasible.\n",e);
					e = -1.0;
					break;
				case MCP_Error:                /* An error occurred within the code            */
					printf("  PATH(%0.4f) failed: an error occurred in the code.\n",e);
					e = -1.0;
					break;
				case MCP_Reduced:		     /* Presolve reduced problem                     */
					printf("  PATH(%0.4f) failed: presolve reduced problem.\n",e);
					e = -1.0;
					break;
				case MCP_LicenseError:	     /* License could not be found                   */
					printf("  PATH(%0.4f) failed: no license.\n",e);
					e = -1.0;
					break;
				default:
					printf("  PATH(%0.4f) failed.\n",e);
					e = -1.0;
					break;
		...
		if( (int)status == 1 ) { soscflag = evalPEQSOSC( ppom , soscinfo , NULL ); }
		else { soscflag = 0; }
		soscflagP = 1;
		
		printf("CG 'MCP' Trial %i -- PATH terminated with status %i\n",t+1,(int)status);
		if( soscflag == 1 ) {
			printf("CG 'MCP' Trial %i -- PATH found a local equilibria\n",t+1);
		} else {
			printf("CG 'MCP' Trial %i -- PATH did not find a local equilibrium:\n",t+1);
			for( f = 0 ; f < F ; f++ ) {
				printf("CG 'MCP' Trial %i -- firm %i: (%i,%i,%i)\n",t+1,f+1,ppom[f+1],soscinfo[f],soscinfo[F+f]);
		...
		if( rfp != NULL ) {
			...
			// then iterations required (major then minor then regularization)
			fprintf(rfp,"%i,",totmajiters);
			fprintf(rfp,"%i,",totminiters);
			fprintf(rfp,"%i,",triters);
			
			// then time required (total then, % time spent in factorizations)
			fprintf(rfp,"%0.16f,",tottime);
			fprintf(rfp,"%0.16f,",100.0*(tottime-totbasistime)/tottime);
			
			// then status
			fprintf(rfp,"%i,",(int)status);
	...
	for( i = 1 ; i < argc ; i++ ) {
		...
		if( (argv[i])[0] == '-' ) {
			...
			switch( (argv[i])[1] ) {
				...
				case 'R': // initial Tikhonov regularization parameter
					e0 = (double)strtod( argv[i] + 2 , NULL );
					if( e0 < 0.0 ) { e0 = 0.0; }
					break;
